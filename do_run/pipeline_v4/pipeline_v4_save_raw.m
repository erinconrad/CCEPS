...
NEW CODE 1
%}
elecs = get_raw_sweeps_and_avg(bipolar_values,elecs,stim);
...
%% Get monotonic trends
%{
NEW CODE 2
%} 
[ccep_sweeps_processed,N1_trends,N2_trends] = get_monotonic_trends(out);
out.ccep_sweeps_processed = ccep_sweeps_processed;
out.N1_trends = N1_trends;
out.N2_trends = N2_trends;
...
function elecs = get_raw_sweeps_and_avg(values,elecs,stim)
...
for ich = 1:length(elecs)
    ...
    elecs(ich).times = time_to_take;
    elecs(ich).all_bad = zeros(size(values,2),1);
    elecs(ich).raw_sweeps = nan(idx(1,2)-idx(1,1)+1,size(values,2),length(arts));
    ...
    for jch = 1:size(values,2)
       % fprintf('\n   Doing subchannel %d of %d',jch,size(values,2));
        ...
        %% Save raw sweeps
        %eeg bits is nsweeps x nresponse
        %nsamples x nresponse x nsweeps
        elecs(ich).raw_sweeps(:, jch, :) = permute(eeg_bits, [2 3 1]);
...
function [ccep_sweeps_processed,N1_trends,N2_trends] = get_monotonic_trends(out)
    ...
    for j = 1:nchans
        ...
        if stim_trode ~= resp_trode
            ...
            for s_i = 1:size(sweeps_raw,2)
                ...
                %Apply LPF at 30Hz
                x(isnan(x)) = 0; % Erin added this to handle nans
