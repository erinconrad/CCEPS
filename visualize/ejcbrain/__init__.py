...
class node_to_vertex:
	...
	def __init__(self,annot_path,node_data,numnets=7):
	...
	def getvertdata_Yeo(self,hemi=''):
		...
		annot_fname = opj(self.annot_path,hemi+'.Schaefer2018_'+str(nparc)+'Parcels_' + str(self.numnets) + 'Networks_order.annot')
		labels, ctab, names = nib.freesurfer.read_annot(annot_fname)
...
def getannot_Yeo(nparc,hemi='',annot_path='../data/annot',numnets=7):
	annot_fname = opj(annot_path,hemi+'.Schaefer2018_'+str(nparc)+'Parcels_' + str(numnets) + 'Networks_order.annot')
	labels, ctab, names = nib.freesurfer.read_annot(annot_fname)
...
def surfplot_2x2(nodeData,atlas,savedir,ttl='',clim_type='all',cmin=np.nan,cmax=np.nan,cmap='plasma',surf='pial',cortex='low_contrast'):
	# INPUTS:
	...
	# savedir: where to output temporary files which will be deleted
	# clim_type: optional limit for color axis, symmetric about 0. 
	# cmin: optional color minimum
	# cmax: optional color axis maximum
	...
	if clim_type == 'all':
		...
		cmin = -clim
		cmax = clim
	elif ~np.isnan(cmin) & ~np.isnan(cmax):
		cmin=cmin
	else:
		cmin = -clim_type
		cmax = clim_type
	...
	for hemi in hemis:
		...
		for view in views:  
			fig = my.mlab.figure(size=(340,340))
			fig = my.mlab.gcf()
			brain = surfer.Brain(subject_id, hemi, surf,figure=fig,views=view,background='white', alpha = 1, cortex=cortex)
			brain.add_data(vtx_data, min = cmin, max = cmax, thresh = thrsh, colormap=cmap, alpha=.8,colorbar=cbar,time_label=None)    
...
def surfplot_2x2xk(nodeData,atlas,ttls,savedir,clim_type='each',cmin=np.nan,cmax=np.nan,cmap = 'plasma',surf='pial',cortex='low_contrast'):
	...
	for j in np.arange(num_brains): # make full brain 2x2 plots
		f=surfplot_2x2(nodeData[:,j],atlas,savedir,ttl=ttls[j],clim_type=clim_type,cmin=cmin,cmax=cmax,cmap=cmap,surf=surf,cortex='low_contrast')
