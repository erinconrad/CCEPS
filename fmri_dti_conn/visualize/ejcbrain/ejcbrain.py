...
def surfplot_2x2(nodeData,atlas,savedir,ttl='',clim_type='all',cmin,cmax,cmap='plasma',surf='pial',cortex='low_contrast'):
	# INPUTS:
	...
	if clim_type == 'all':
		...
		cmin = -clim
		cmax = clim
	else if ~np.isnan(cmin) & ~np.isnan(cmax):
		continue
	else:
		cmin = -clim_type
		cmax = clim_type
...
def surfplot_2x2xk(nodeData,atlas,ttls,savedir,clim_type='each',cmin=np.nan,cmax=np.nan,cmap = 'plasma',surf='pial',cortex='low_contrast'):
	...
	for j in np.arange(num_brains): # make full brain 2x2 plots
		f=surfplot_2x2(nodeData[:,j],atlas,savedir,ttl=ttls[j],clim_type=clim_type,cmin,cmax,cmap=cmap,surf=surf,cortex='low_contrast')
